...
# ==============================================================================
# FASE DE VERIFICACIÓN: Generar la "Golden Reference" con 1 proceso
# ==============================================================================
echo "⏳ Generando Referencia Maestra (1 proceso) para validación estricta..."
salloc -p jutjat -N 1 --exclusive bash -c "srun -N 1 -n 1 --ntasks-per-node=1 --distribution=block:block ./kmean_p | grep '^R\[' > ref.out"
...
for config in "${CONFIGS[@]}"; do
    ...
    # Executem guardant la sortida 
    OUTPUT=$(salloc -p jutjat -N $nodes --exclusive bash -c "TIMEFORMAT='TIEMPO_REAL:%R'; time srun -N $nodes -n $procs --ntasks-per-node=$ppn --distribution=block:block $OVERCOMMIT_FLAG ./kmean_p" 2>&1)
